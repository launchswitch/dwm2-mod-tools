...
**Implication:** Combat *representation* (instantiate a battle, populate stats/skills/resistances) is fully portable. Combat *resolution* (damage formula, crits, evasion, resistance multipliers, status durations, per-skill mechanics, turn order, AI quality) is now **almost entirely documented** — see `mechanics/mcbick_guide/` and `mechanics/stat_combat_roles.md`. The only remaining [Unknown] of significance is the **courage/motivation gain curve** (1-3 points per command, level-dependent, but the level→rate formula isn't numerically specified). For that and for the specific tier-thresholds flagged as "design call" above, MonTamer will use original design informed by the maintainer's knowledge.
